...
bool is_path_safe(const string& root_dir, const string& test_file) {
  ...
  char* ptr = realpath(test_file.c_str(), abs_path);
  if (ptr == nullptr) {
    return false;
  }
  ...
  string cwd_string = cwd.string();
  path.erase(0, cwd_string.length() + 1);

  // given absolute path, check if under root dir
  string new_root_dir = root_dir;
  new_root_dir.append("/");
  if (path.substr(0, root_dir.length() + 1).compare(new_root_dir) == 0) {
    return true;
  } else {
    return false;
  }
...
string escape_html(const string& from) {
  ...
  // TODO: implement
  const string unsafe_one{"&"};
  const string unsafe_two{">"};
  const string unsafe_three{"<"};
  const string unsafe_four{"'"};
  const string unsafe_five{'"'};

  boost::replace_all(ret, unsafe_one, "&amp;");
  boost::replace_all(ret, unsafe_two, "&gt;");
  boost::replace_all(ret, unsafe_three, "&lt;");
  boost::replace_all(ret, unsafe_four, "&apos;");
  boost::replace_all(ret, unsafe_five, "&quot;");
  return ret;
}
