...
static void handle_file(const string& fpath, WordIndex* index) {
  ...
  boost::split(words, str, !boost::is_alpha(), boost::token_compress_on);
  // Record each non empty token as a word into the Wordindex specified by
  // *index Your implementation should also be case in-sensitive and record
  // every word in all lower-case
  for (auto& word : words) {
    string lower_word = boost::to_lower_copy(word);
    if (lower_word.empty()) {
      continue;
    }
    index->record(lower_word, fpath);
  }
}
